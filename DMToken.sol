...
contract DividendPayingToken is ERC20, Ownable, DividendPayingTokenInterface, DividendPayingTokenOptionalInterface {
  ...
  address public CAKE = address(0x08ba0619b1e7A582E0BCe5BBE9843322C954C340); //CAKE
  ...
  function distributeCAKEDividends(uint amount) public onlyOwner{
...
contract DMAN is ERC20, Ownable {
    ...
    DMANDividendTracker public dividendTracker;

    address public deadWallet = 0x000000000000000000000000000000000000dEaD;

    address public CAKE = address(0x08ba0619b1e7A582E0BCe5BBE9843322C954C340); //CAKE

    uint public swapTokensAtAmount = 2000000 * (10**18);
    
    mapping(address => bool) public _isBlacklisted;

    uint public CAKERewardsFee = 7;
    uint public liquidityFee = 3;
    uint public marketingFee = 5;
    uint public totalFees = CAKERewardsFee.add(liquidityFee).add(marketingFee);
    ...
    constructor() public ERC20("DMAN Swap", "DMAN") {

    	dividendTracker = new DMANDividendTracker();
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "DMAN: The dividend tracker already has that address");

        DMANDividendTracker newDividendTracker = DMANDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "DMAN: The new dividend tracker must be owned by the DMAN token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "DMAN: The router already has that address");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "DMAN: Account is already the value of 'excluded'");
    ...
    function setCAKERewardsFee(uint value) external onlyOwner{
    ...
    function setMarketingFee(uint value) external onlyOwner{
        marketingFee = value;
        totalFees = CAKERewardsFee.add(liquidityFee).add(marketingFee);
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "DMAN: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "DMAN: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "DMAN: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "DMAN: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        if( canSwap &&
            ...
            uint marketingTokens = contractTokenBalance.mul(marketingFee).div(totalFees);
    ...
    function swapAndSendToFee(uint tokens) private  {

        uint initialCAKEBalance = IERC20(CAKE).balanceOf(address(this));

        swapTokensForCake(tokens);
        uint newBalance = (IERC20(CAKE).balanceOf(address(this))).sub(initialCAKEBalance);
    ...
    function swapTokensForCake(uint tokenAmount) private {
    ...
    function swapAndSendDividends(uint tokens) private{
        swapTokensForCake(tokens);
        uint dividends = IERC20(CAKE).balanceOf(address(this));
...
contract DMANDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint;
    using SafeMathInt for int256;
    ...
    uint public minimumTokenBalanceForDividends;
    ...
    constructor() public DividendPayingToken("DMAN_Dividen_Tracker", "DMAN_Dividend_Tracker") {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 200000 * (10**18); //must hold 200000+ tokens
    }

    function _transfer(address, address, uint) internal override {
        require(false, "DMAN_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "DMAN_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main DMAN contract.");
    }
    ...
    function updateClaimWait(uint newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "DMAN_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "DMAN_Dividend_Tracker: Cannot update claimWait to same value");
